...
APP.modules.map = (function() {
    ...
    return {
        ...
        init: function(htmlContainer) {
            ...
            var ign = L.tileLayer(
                'https://wxs.ign.fr/t1uc0ubue3xepf1zshnwnqmw/geoportail/wmts?service=WMTS&request=GetTile&version=1.0.0&tilematrixset=PM&tilematrix={z}&tilecol={x}&tilerow={y}&layer=ORTHOIMAGERY.ORTHOPHOTOS&format=image/jpeg&style=normal', {
                    minZoom: 0,
                    maxZoom: 18,
                    tileSize: 256,
            ...
            if (data == null || data.length == 0) {
                ...
                $.map(data, function(k2, v2) {
                    ...
                    if (k2.COORDINATES) {
                        ...
                        console.log('k2.COORDINATES.LONG : ' + k2.COORDINATES.LONG)
			    ...
			    */
			    // NOMETAL DEB
                            if (k.SUPPLEMENTARY_FIELDS.SAMPLE_KIND != null && k.SUPPLEMENTARY_FIELDS.SAMPLE_KIND != '') {
                                lithology[k.SUPPLEMENTARY_FIELDS.SAMPLE_ORIGIN] = (k.SUPPLEMENTARY_FIELDS.SAMPLE_ORIGIN);
                                //lithology[k.SAMPLING_POINT[0].NAME] = (k.SAMPLING_POINT[0].NAME);
                            }
                            if (k.SUPPLEMENTARY_FIELDS.MEASUREMENT_TYPE != null && k.SUPPLEMENTARY_FIELDS.MEASUREMENT_TYPE != '') {
                                lithology3[k.SUPPLEMENTARY_FIELDS.SAMPLE_KIND] = (k.SUPPLEMENTARY_FIELDS.SAMPLE_KIND);
                            }
			    // NOMETAL FIN
    ...
    return {
        ...
        check_clicked: function(e, name_CSRF, value_CSRF, name, firstname, mail, type, project_name, project_name_modify) {
            ...
            if (action == 'approve') {
                ...
                $(".modal.user .content").empty();
                $(".modal.user .header").append('Modify informations: ' + mail);
                $(".modal.user .content").append('<form class="ui large form myaccount" method="post" action="/modifyuser"><input type="hidden" name="csrf_name" value="' + name_CSRF + '"><input type="hidden" name="csrf_value" value="' + value_CSRF + '"><input type="hidden" name="email" value="' + mail + '"><input type="hidden" name="project_name_modify" value="' + project_name_modify + '"> <div class="ui error message"></div> <div class="field"><div class="ui selection dropdown"><input type="hidden" name="type"><i class="dropdown icon"></i><div class="default text">Access right</div><div class="menu"><div class="item" data-value="0">User</div></div></div></div></div></div> </div> <div class="actions"><div class="ui black deny button"> Cancel </div> <button class="ui submit red button" >Yes</button> </div></div> </form>');
                $('.ui.modal.user').modal('show');
                $('.ui.dropdown').dropdown('set selected', type);
                APP.modules.account.check_myaccount();
                ...
                $(".modal.user .content").empty();
                $(".modal.user .header").append('Modify informations: ' + mail);
                $(".modal.user .content").append('<form class="ui large form myaccount" method="post" action="/modifyuser"><input type="hidden" name="csrf_name" value="' + name_CSRF + '"><input type="hidden" name="csrf_value" value="' + value_CSRF + '"><input type="hidden" name="email" value="' + mail + '"><input type="hidden" name="project_name_modify" value="' + project_name_modify + '"> <div class="ui error message"></div> <div class="field"><div class="ui selection dropdown"><input type="hidden" name="type"><i class="dropdown icon"></i><div class="default text">Access right</div><div class="menu"><div class="item" data-value="0">User</div><div class="item" data-value="2">Referent</div></div></div></div></div></div> </div> <div class="actions"><div class="ui black deny button"> Cancel </div> <button class="ui submit red button" >Yes</button> </div></div> </form>');
                $('.ui.modal.user').modal('show');
                $('.ui.dropdown').dropdown('set selected', type);
                APP.modules.account.check_myaccount();
        ...
        add_user: function(project_name) {
            $.post("add_user_projects", {
                mail_user: $("input[type=text][name=usermail_add_" + project_name + "]").val(),
