...
class RequestController
{
    ...
    {
        $this->filter1   = "INTRO.SUPPLEMENTARY_FIELDS.SAMPLE_ORIGIN";
        $this->filter2   = "INTRO.SUPPLEMENTARY_FIELDS.SAMPLE_KIND";
    }
    ...
    function Request_data_awaiting($id)
    {
        ...
        $bdd     = strtolower($config['MONGO_DATABASE']);
        $url     = 'http://' . "GLOBAL_elasticsearch" . '/' . $bdd . '/' . $config['COLLECTION_NAME'] . '_sandbox/' . $id;
        $curlopt = array(
    ...
    function Request_all_data_awaiting()
    {
        ...
        $config  = $file->ConfigFile();
        $bdd     = strtolower($config['MONGO_DATABASE']);
        $url     = 'http://' . "GLOBAL_elasticsearch" . '/' . $bdd . '/_search?type=' . $config['COLLECTION_NAME'] . '_sandbox&size=10000';
        $curlopt = array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_PORT           => "9200",
            CURLOPT_ENCODING       => "",
    ...
    function pass_es(){
        ...
        $url = "GLOBAL_elasticsearch" . '/' . strtolower($config['MONGO_DATABASE']) . "/_search?type=" . $config['COLLECTION_NAME'] . "&size=10000";
        $postcontent = '{ "_source": { 
            ...
            "INTRO.SAMPLING_DATE","INTRO.SAMPLING_POINT","INTRO.MEASUREMENT","INTRO.KEYWORDS","DATA.FILES" ] 
        }}';
        $curlopt = array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
    ...
    function Request_all_poi()
    {
        ...
        foreach ($response as $key => $value) {
            ...
            // Regrouper les points par coord
            if (!$return[$value['_source']['INTRO']['SAMPLING_POINT'][0]['LATITUDE'] . '/' . $value['_source']['INTRO']['SAMPLING_POINT'][0]['LONGITUDE'][$value['_source']['INTRO']['SUPPLEMENTARY_FIELDS']['SAMPLE_NAME']]]) {
                ...
                // ----- NOMETAL DEB
                //$return[$value['_source']['INTRO']['SAMPLING_POINT'][0]['LATITUDE'].'/'.$value['_source']['INTRO']['SAMPLING_POINT'][0]['LONGITUDE']][$value['_source']['INTRO']]['KEYWORDS'][0]['NAME'] = $value['_source']['INTRO']['KEYWORDS'][0]['NAME'];
                // ----- NOMETAL FIN
    ...
    function Request_data_with_sort($sort)
    {
        ...
        $config = self::ConfigFile();
        $url = "GLOBAL_elasticsearch" . '/' . strtolower($config['MONGO_DATABASE']) . "/_search?q=" . $lithology . $lithology3 . $mesure . $date . $geo . "type=" . $config['COLLECTION_NAME'] . "&sort=INTRO.SAMPLING_DATE&size=10000";
                ...
                if (strtoupper($sort['mesure']) == strtoupper($value['_source']['INTRO']['MEASUREMENT'][0]['ABBREVIATION'])) {
                    if ($value['_source']['DATA']['FILES'][0]['TYPE_DATA'] == 'Data') {
                        ...
                        if ($file_parts['extension'] == "xlsx") {
                            $CSV_FOLDER = $config["IMPORT_FOLDER"];
        ...
        /* if ($sort['lithology'])
    {
       $lithology = 'INTRO.SUPPLEMENTARY_FIELDS.LITHOLOGY:"' . urlencode($sort['lithology']) . '"%20AND%20';
        ...
        $config = self::ConfigFile();
        $url = "GLOBAL_elasticsearch" . '/' . strtolower($config['MONGO_DATABASE']) . "/_search?q=" . $lithology . $lithology3 . $mesure . $date . "type=" . $config['COLLECTION_NAME'] . "&sort=INTRO.SAMPLING_DATE&size=10000";
    ...
    function Request_poi_data_awaiting($id, $name)
    {
        ...
        $config = self::ConfigFile();
        $url = "GLOBAL_elasticsearch" . '/' . strtolower($config['MONGO_DATABASE']) . '/_search?q=(INTRO.MEASUREMENT.ABBREVIATION:"' . $explode[1] . '"%20AND%20INTRO.SUPPLEMENTARY_FIELDS.SAMPLE_NAME:"' . $explode[0] . '")&type=' . $config['COLLECTION_NAME'] . '_sandbox';
        $curlopt = array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_PORT => "9200",
    ...
    function Request_poi_data($id)
    {
        ...
        $config = self::ConfigFile();
        $url = "GLOBAL_elasticsearch" . '/' . strtolower($config['MONGO_DATABASE']) . '/_search?q=(INTRO.MEASUREMENT.ABBREVIATION:"' . $explode[1] . '"%20AND%20INTRO.SUPPLEMENTARY_FIELDS.SAMPLE_NAME:"' . $explode[0] . '")&type=' . $config['COLLECTION_NAME'];
        $curlopt = array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_PORT => "9200",
    ...
    {
        $config = self::ConfigFile();
        $url = "GLOBAL_elasticsearch" . '/' . strtolower($config['MONGO_DATABASE']) . '/_search?q=INTRO.MEASUREMENT.ABBREVIATION:"' . $id . '"&type=' . $config['COLLECTION_NAME'];
        $curlopt = array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_PORT => "9200",
    ...
    function Post_Processing($POST, $route)
    {
        ...
        $UPLOAD_FOLDER    = $config["IMPORT_FOLDER"];
        ...
        foreach ($POST as $key => $value) {
            ...
            switch ($key) {
                ...
                case "sample_name":
                    ...
                    try {
                        if (empty($config['MONGO_DATABASE']) && empty($config['MONGO_INIT_DB_USER']) && empty($config['MONGO_INIT_DB_USER_PASS'])) {
                            $this->db = new MongoDB\Driver\Manager("mongodb://" . 'GLOBAL_mongo' . ':' . '27017', array('journal' => false));
                        } else {
                            $this->db = new MongoDB\Driver\Manager("mongodb://" . 'GLOBAL_mongo' . ':' . '27017', array('journal' => false, 'authSource' => $config['MONGO_DATABASE'], 'username' => $config['MONGO_INIT_DB_USER'], 'password' => $config['MONGO_INIT_DB_USER_PASS']));
                        }
                    } catch (Exception $e) {
                        echo $e->getMessage();
                    ...
                    try {
                        ...
                        $cursor = $this->db->executeQuery($config['MONGO_DATABASE'] . '.' . $config['COLLECTION_NAME'], $query);
        ...
        if (!$error == null) {
            ...
            if ($route == 'modify') {
                ...
                // var_dump($filter);
                $query = new MongoDB\Driver\Query($filter);
                $cursor = $this->db->executeQuery($config['MONGO_DATABASE'] . '.' . $config['COLLECTION_NAME'] . '_sandbox', $query);
                foreach ($cursor as $document) {
                ...
                try {
                    ...
                    $this->db->executeBulkWrite($config['MONGO_DATABASE'] . '.' . $config['COLLECTION_NAME'], $bulk);
                    $bulk = new MongoDB\Driver\BulkWrite;
                    $bulk->delete(['_id' => strtoupper($_POST['original_sample_name'])]);
                    $this->db->executeBulkWrite($config['MONGO_DATABASE'] . '.' . $config['COLLECTION_NAME'] . '_sandbox', $bulk);
                    $repertoireDestination         = $UPLOAD_FOLDER;
                    mkdir($repertoireDestination  . "/" . $_POST['sample_name'] . "_META");
